...
SparseMatrix ComputeEmpiricalTransitionMatrix(const TrailsSlice& trails,
  ...
  FILE *fp;
  fp = fopen("transition_matrix.txt", "w");
  fprintf(fp, "%s", "SOF\n");
  fclose(fp);

  Normalize(&transitions);
  save_to_file(&transitions);

  return transitions;
}


void save_to_file(SparseMatrix* transitions) {
  // fclose(fp);
  int i;
  i=0;
  for (SparseVector& row : *transitions) {
  ...
  FILE *fp;
  fp = fopen("transition_matrix.txt", "a");
  fprintf(fp, "%s", "EOF");
  fclose(fp);
}

void save_row_int(SparseVector* v, int i) { save_row(*v, i); }

void save_row(const SparseVector& v, int i) { 
  int j;
  j=0;
  for (const auto& item_and_value : v) {
    FILE *fp;
...
// int save_to_file (unsigned n, unsigned m, SparseMatrix mat, const char *f){
//     // FILE *fp;

//   std::ofstream fout("transition_matrix.txt");
...
//   // fclose(fp);
//   return 0;
//     //             fprintf(fp, "%f,%d,%d", mat[i][j], i, j);
    
// // https://stackoverflow.com/questions/63947319/writing-and-loading-a-large-array-to-from-a-file-in-c

//   // {
//   //   std::ofstream out("m_trans.data", std::ios_base::int);
...
int write_one(int i, int j, double val, const char *f){
    FILE *fp;
    fp = fopen(f, "a");
    fprintf(fp, "%f,%d,%d\n", val, i, j);
    fclose(fp);
